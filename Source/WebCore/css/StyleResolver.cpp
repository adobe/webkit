...
static WebKitCSSShaderValue* cssShaderValue(CSSValue* value)
{
    if (value->isWebKitCSSShaderValue())
...
PassRefPtr<CustomFilterOperation> StyleResolver::createCustomFilterOperationWithInlineSyntax(WebKitCSSFilterValue* filterValue)
{
    ...
    WebKitCSSShaderValue* vertexShader = cssShaderValue(shadersList->itemWithoutBoundsCheck(0));
    ...
    if (shadersListLength > 1) {
        ...
        if (fragmentShaderOrMixFunction->isWebKitCSSMixFunctionValue()) {
            ...
            fragmentShader = cssShaderValue(iterator.value());
            iterator.advance();
            ...
            fragmentShader = cssShaderValue(fragmentShaderOrMixFunction);
        }
    }
