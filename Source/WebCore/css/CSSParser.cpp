...
PassRefPtr<CSSValueList> CSSParser::parseTransform()
{
    ...
    for (CSSParserValue* value = m_valueList->current(); value; value = m_valueList->next()) {
        ...
        TransformOperationInfo info(value->function->name, m_context.mode);
        if (info.unknown())
            return 0;

        if (!info.hasCorrectArgCount(args->size()) && m_context.mode != SVGAttributeMode)
            return 0;
        ...
        while (a) {
            ...
            if (info.type() == WebKitCSSTransformValue::RotateTransformOperation && argNumber > 0) {
                ...
                if (!validUnit(a, unit, m_context.mode))
                    return 0;
                ...
                if (!validUnit(a, FAngle, m_context.mode))
                    return 0;
                ...
                if (!validUnit(a, FLength, m_context.mode))
                    return 0;
                ...
                if (!validUnit(a, FNumber | FLength | FNonNeg, m_context.mode))
                    return 0;
            } else if (!validUnit(a, unit, m_context.mode))
                return 0;
